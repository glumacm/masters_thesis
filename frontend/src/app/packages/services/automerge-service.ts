...
export class SyncLibAutoMerge {
    ...
    applyConflictPatch(conflict: SyncConflictItem, beMergedData: any, localData: any, useRemote: boolean = true): any {
        ...
        let dataToReturn = clonedLocalData;
        ...
        // op.value = beMergedData?.[conflict.fieldName];
        if(beMergedData?.[CONFIGURATION_CONSTANTS.LAST_MODIFIED_FIELD]) {
            dataToReturn[CONFIGURATION_CONSTANTS.LAST_MODIFIED_FIELD] = beMergedData[CONFIGURATION_CONSTANTS.LAST_MODIFIED_FIELD];
        }
        // dataToReturn = this.applyPatch(clonedBeData, [op]);
        if (!useRemote) { // === USE-LOCAL
            op.value = localData?.[conflict.fieldName];
        }
        dataToReturn = this.applyPatch(clonedLocalData, [op]);
